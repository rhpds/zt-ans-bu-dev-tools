...
. *Open the project folder in VS Code.* In the top menu, select **File** → **Open Folder...**, enter the following path and click **OK**:
+
...
----
+
* **Base image:** In the base image field, enter `quay.io/centos/centos:stream10` to use a public CentOS Stream 10 image that does not require registry authentication.
...
[NOTE]
====
When using Red Hat supported EE base images from `registry.redhat.io` (e.g. `ee-minimal-rhel8`), you must add `package_manager_path: /usr/bin/microdnf` to the `options:` section, as those images are built on UBI-minimal which uses `microdnf` instead of `dnf`.
====
