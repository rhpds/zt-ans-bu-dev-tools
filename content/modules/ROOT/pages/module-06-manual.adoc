...
+
In the VS Code file explorer view, **right click** the `mycollection` folder, and create a new file named `execution-environment.yml`.
...
. *Now create the `requirements.yml` file:*
+
In the your `mycollection` directory again, right click and create a new file named `requirements.yml` and paste the following content:
+
[source,yaml]
----
---
...
----
+
This way `ansible-builder` will include your local collection automatically since you will be building from within the collection directory - you just need the `requirements.yml` file to exist with valid (even if empty) content to satisfy the build process. 

. *Now let's add the `cowsay` dependency:*
+
In the your `mycollection` directory again you will find a `requirements.txt` file (note the extension, this is the txt one!), open it, it will be empty. Add the `cowsay` package to it. 
+
[source,yaml]
----
---
...
----
+
NOTE: As we are using a RHEL UBI image as a base for our Execution Environment, cowsay is not available as a system package.
...
. *Make sure you are in the mycollection directory:*
+
Ensure your terminal is operating in the directory containing your collection.
+
[source,bash]
----
cd /home/rhel/myansibleproject/collections/ansible_collections/mynamespace/mycollection/
