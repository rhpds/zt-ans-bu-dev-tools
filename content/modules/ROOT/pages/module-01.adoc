...
.   Due to workshop limitations we are going to upgrade `ansible-dev-tools` manually and pin the `ansible-core` version. In the *VS Code top menu*, click *Terminal* and *New terminal*.
+
image::vscode-devtools-terminal.png[Reload extensions in VS Code, opts="border"]

..   Now in the *VS Code Terminal* run the following commands:
+
...
----
..   You can close the Terminal in VS Code now
...
.   *Copy & Pasting into the VS Code Terminal*: There is a known issue with the workshop setup while trying to paste into the VS Code Terminal:
+
image::vscode-devtools-extension-reload.png[Reload extensions in VS Code, opts="border"]
+
..  **Chrome:** The key combo of **Control + Shift + V** (Mac: **Command + Shift + V**) should work for pasting. Another workaround requires using Firefox.
..  **Firefox:** The key combo of **Control + Shift + V** (Mac: **Command + Shift + V**). There is another workaround by using the right-click menu for *Paste* into the Terminal (and if using Linux, you might use the selection-buffer and middle-click).
