...
image::vscode-devtools-extension-reload.png[Reload extensions in VS Code, opts="border"]
+
..  **Chrome** you might need to type the CLI commands into the VS Code Terminal. The workaround is to use Firefox :) 
..  **Firefox** has a workaround by using the right-click menu for *Paste* into the Terminal (and if using Linux, you might use the selection-buffer and middle-click).
